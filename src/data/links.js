// Central relationship map for internal linking across reviews, guides, and playbooks.
// Edit this file to adjust which reviews are "related" and which playbooks reference each tool.

// --- Guides index (for review → guide links) ---
export const guides = {
  'ai-tools-side-hustlers': {
    slug: 'ai-tools-side-hustlers',
    title: 'Best AI Tools for Side Hustlers in 2026',
  },
  'web-hosting-beginners': {
    slug: 'web-hosting-beginners',
    title: 'Best Web Hosting for Beginners',
  },
  'vpn-remote-workers': {
    slug: 'vpn-remote-workers',
    title: 'Best VPNs for Remote Workers & Travelers',
  },
  'productivity-apps': {
    slug: 'productivity-apps',
    title: 'Best Productivity Apps Worth Paying For',
  },
  'saas-small-teams': {
    slug: 'saas-small-teams',
    title: 'Best SaaS Tools for Small Teams',
  },
  'online-courses-high-income': {
    slug: 'online-courses-high-income',
    title: 'Best Online Courses for High-Income Skills',
  },
  'ai-for-blog-writing': {
    slug: 'ai-for-blog-writing',
    title: 'Best AI Tools for Blog Writing in 2026',
  },
  'ai-for-coding': {
    slug: 'ai-for-coding',
    title: 'Best AI Tools for Coding in 2026',
  },
  'vpn-for-streaming-netflix': {
    slug: 'vpn-for-streaming-netflix',
    title: 'Best VPNs for Streaming Netflix in 2026',
  },
  'hosting-for-wordpress': {
    slug: 'hosting-for-wordpress',
    title: 'Best Hosting for WordPress in 2026',
  },
}

// --- Playbooks index ---
export const playbooks = {
  'release-a-song-with-suno': {
    slug: 'release-a-song-with-suno',
    title: 'How to Release a Song with Suno',
  },
  'launch-a-side-project-with-ai': {
    slug: 'launch-a-side-project-with-ai',
    title: 'How to Launch a Side Project with AI',
  },
  'learn-a-skill-with-udemy': {
    slug: 'learn-a-skill-with-udemy',
    title: 'How to Learn a High-Income Skill with Udemy',
  },
  'post-30-videos-with-invideo-fiverr': {
    slug: 'post-30-videos-with-invideo-fiverr',
    title: 'How to Post 30 Videos with InVideo & Fiverr',
  },
}

// --- Reviews index (for cross-linking) ---
export const reviewMeta = {
  'claude-ai':   { slug: 'claude-ai',   name: 'Claude AI',    category: 'AI Tools' },
  'hostinger':   { slug: 'hostinger',   name: 'Hostinger',    category: 'Hosting' },
  'nordvpn':     { slug: 'nordvpn',     name: 'NordVPN',      category: 'VPN' },
  'notion':      { slug: 'notion',      name: 'Notion',       category: 'Productivity' },
  'grammarly':   { slug: 'grammarly',   name: 'Grammarly',    category: 'Productivity' },
  'suno':        { slug: 'suno',        name: 'Suno',         category: 'AI Tools' },
  'coursera':    { slug: 'coursera',    name: 'Coursera',     category: 'Courses' },
  'skillshare':  { slug: 'skillshare',  name: 'Skillshare',   category: 'Courses' },
  'udemy':       { slug: 'udemy',       name: 'Udemy',        category: 'Courses' },
  'n8n':         { slug: 'n8n',         name: 'n8n',          category: 'SaaS' },
  'fiverr':      { slug: 'fiverr',      name: 'Fiverr',       category: 'SaaS' },
  'invideo':     { slug: 'invideo',     name: 'InVideo AI',   category: 'AI Tools' },
  'chatgpt':     { slug: 'chatgpt',     name: 'ChatGPT',      category: 'AI Tools' },
  'gemini':      { slug: 'gemini',      name: 'Gemini',       category: 'AI Tools' },
  'perplexity':  { slug: 'perplexity',  name: 'Perplexity',   category: 'AI Tools' },
  'expressvpn':  { slug: 'expressvpn',  name: 'ExpressVPN',   category: 'VPN' },
  'surfshark':   { slug: 'surfshark',   name: 'Surfshark',    category: 'VPN' },
  'protonvpn':   { slug: 'protonvpn',   name: 'Proton VPN',   category: 'VPN' },
  'bluehost':    { slug: 'bluehost',    name: 'Bluehost',     category: 'Hosting' },
  'siteground':  { slug: 'siteground',  name: 'SiteGround',   category: 'Hosting' },
  'cloudways':   { slug: 'cloudways',   name: 'Cloudways',    category: 'Hosting' },
  'zapier':      { slug: 'zapier',      name: 'Zapier',       category: 'SaaS' },
  'airtable':    { slug: 'airtable',    name: 'Airtable',     category: 'SaaS' },
  'webflow':     { slug: 'webflow',     name: 'Webflow',      category: 'SaaS' },
}

// --- Per-review relationship map ---
// relatedGuides: guides this review appears in / belongs to
// relatedReviews: 2-3 alternatives or complementary tools
// relatedPlaybooks: playbooks that use this tool
export const reviewLinks = {
  'claude-ai': {
    relatedGuides: ['ai-tools-side-hustlers', 'productivity-apps', 'saas-small-teams'],
    relatedReviews: ['notion', 'grammarly', 'n8n'],
    relatedPlaybooks: ['launch-a-side-project-with-ai'],
  },
  'hostinger': {
    relatedGuides: ['web-hosting-beginners', 'hosting-for-wordpress'],
    relatedReviews: ['bluehost', 'siteground', 'cloudways'],
    relatedPlaybooks: [],
  },
  'nordvpn': {
    relatedGuides: ['vpn-remote-workers'],
    relatedReviews: [],
    relatedPlaybooks: [],
  },
  'notion': {
    relatedGuides: ['productivity-apps', 'saas-small-teams'],
    relatedReviews: ['claude-ai', 'grammarly', 'n8n'],
    relatedPlaybooks: ['launch-a-side-project-with-ai'],
  },
  'grammarly': {
    relatedGuides: ['productivity-apps', 'ai-tools-side-hustlers'],
    relatedReviews: ['claude-ai', 'notion'],
    relatedPlaybooks: [],
  },
  'suno': {
    relatedGuides: ['ai-tools-side-hustlers'],
    relatedReviews: ['invideo', 'claude-ai'],
    relatedPlaybooks: ['release-a-song-with-suno'],
  },
  'coursera': {
    relatedGuides: ['online-courses-high-income'],
    relatedReviews: ['skillshare', 'udemy'],
    relatedPlaybooks: [],
  },
  'skillshare': {
    relatedGuides: ['online-courses-high-income'],
    relatedReviews: ['coursera', 'udemy'],
    relatedPlaybooks: [],
  },
  'udemy': {
    relatedGuides: ['online-courses-high-income'],
    relatedReviews: ['coursera', 'skillshare'],
    relatedPlaybooks: ['learn-a-skill-with-udemy'],
  },
  'n8n': {
    relatedGuides: ['saas-small-teams'],
    relatedReviews: ['notion', 'claude-ai'],
    relatedPlaybooks: ['launch-a-side-project-with-ai'],
  },
  'fiverr': {
    relatedGuides: ['ai-tools-side-hustlers'],
    relatedReviews: ['invideo'],
    relatedPlaybooks: ['post-30-videos-with-invideo-fiverr'],
  },
  'invideo': {
    relatedGuides: ['ai-tools-side-hustlers'],
    relatedReviews: ['fiverr', 'suno'],
    relatedPlaybooks: ['post-30-videos-with-invideo-fiverr'],
  },
  'chatgpt': {
    relatedGuides: ['ai-tools-side-hustlers', 'productivity-apps', 'saas-small-teams'],
    relatedReviews: ['claude-ai', 'notion', 'grammarly'],
    relatedPlaybooks: ['launch-a-side-project-with-ai'],
  },
  'gemini': {
    relatedGuides: ['ai-tools-side-hustlers', 'productivity-apps'],
    relatedReviews: ['claude-ai', 'chatgpt', 'notion'],
    relatedPlaybooks: [],
  },
  'perplexity': {
    relatedGuides: ['ai-tools-side-hustlers'],
    relatedReviews: ['claude-ai', 'chatgpt', 'gemini'],
    relatedPlaybooks: [],
  },
  'expressvpn': {
    relatedGuides: ['vpn-remote-workers'],
    relatedReviews: ['nordvpn', 'surfshark', 'protonvpn'],
    relatedPlaybooks: [],
  },
  'surfshark': {
    relatedGuides: ['vpn-remote-workers'],
    relatedReviews: ['nordvpn', 'expressvpn', 'protonvpn'],
    relatedPlaybooks: [],
  },
  'protonvpn': {
    relatedGuides: ['vpn-remote-workers'],
    relatedReviews: ['nordvpn', 'expressvpn', 'surfshark'],
    relatedPlaybooks: [],
  },
  'bluehost': {
    relatedGuides: ['web-hosting-beginners', 'hosting-for-wordpress'],
    relatedReviews: ['hostinger', 'siteground', 'cloudways'],
    relatedPlaybooks: [],
  },
  'siteground': {
    relatedGuides: ['web-hosting-beginners', 'hosting-for-wordpress'],
    relatedReviews: ['hostinger', 'bluehost', 'cloudways'],
    relatedPlaybooks: [],
  },
  'cloudways': {
    relatedGuides: ['web-hosting-beginners', 'hosting-for-wordpress'],
    relatedReviews: ['hostinger', 'siteground', 'bluehost'],
    relatedPlaybooks: [],
  },
  'zapier': {
    relatedGuides: ['saas-small-teams'],
    relatedReviews: ['n8n', 'notion', 'airtable'],
    relatedPlaybooks: ['launch-a-side-project-with-ai'],
  },
  'airtable': {
    relatedGuides: ['saas-small-teams', 'productivity-apps'],
    relatedReviews: ['notion', 'zapier', 'n8n'],
    relatedPlaybooks: [],
  },
  'webflow': {
    relatedGuides: ['saas-small-teams', 'web-hosting-beginners'],
    relatedReviews: ['hostinger', 'notion', 'zapier'],
    relatedPlaybooks: [],
  },
}

// --- Per-guide relationship map ---
// relatedReviews: full reviews for tools featured in this guide
export const guideLinks = {
  'ai-tools-side-hustlers':       { relatedReviews: ['claude-ai', 'invideo', 'notion'] },
  'web-hosting-beginners':        { relatedReviews: ['hostinger'] },
  'vpn-remote-workers':           { relatedReviews: ['nordvpn'] },
  'productivity-apps':            { relatedReviews: ['notion', 'grammarly', 'claude-ai'] },
  'saas-small-teams':             { relatedReviews: ['notion', 'n8n'] },
  'online-courses-high-income':   { relatedReviews: ['coursera', 'skillshare', 'udemy'] },
  'ai-for-blog-writing':          { relatedReviews: ['claude-ai', 'chatgpt', 'grammarly', 'notion', 'perplexity'] },
  'ai-for-coding':                { relatedReviews: ['claude-ai', 'chatgpt', 'gemini', 'perplexity', 'n8n'] },
  'vpn-for-streaming-netflix':    { relatedReviews: ['nordvpn', 'expressvpn', 'surfshark', 'protonvpn'] },
  'hosting-for-wordpress':        { relatedReviews: ['hostinger'] },
}
